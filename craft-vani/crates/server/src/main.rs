...
async fn main() {
    ...
    let model_dir = Path::new("whisper-small.en");
    ...
    let state = Arc::new(Mutex::new(ServerState { decoder }));
...
async fn handle_socket(socket: WebSocket, state: Arc<Mutex<ServerState>>) {
    ...
    tokio::spawn(async move {
        ...
        while let Some(task) = rx.recv().await {
            match task {
                InferenceTask::SetSampleRate(sample_rate) => {
                    ...
                    if let Some(decoder) = &mut state.decoder {
                        match decoder.run_raw(&resampled) {
                            Ok(segments) => {
                                ...
                                let output_segments = if emit { filtered_segments } else { vec![] };
                                println!(
                                    "[Inference] Decoding successful: {} kept / {} total (emit={})",
                                    output_segments.len(),
                                    total_segments,
                                    emit
                                );
        ...
        match msg {
            Message::Text(text) => {
                if let Ok(config) = serde_json::from_str::<WsConfigMessage>(&text)
                    ...
                    && tx.send(InferenceTask::SetSampleRate(sample_rate as usize)).await.is_err()
                {
