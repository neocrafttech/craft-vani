...
impl Decoder {
    ...
    fn new(
        model: Model, tokenizer: Tokenizer, mel_filters: Vec<f32>, device: &Device,
        task: Option<Task>, language: Option<String>, is_multilingual: bool, timestamps: bool,
    ) -> anyhow::Result<Self> {
        let suppress_tokens: Vec<f32> =
            (0..model.config().vocab_size as u32)
                .map(|i| {
    ...
    pub fn convert_and_run(&mut self, wav_input: &[u8]) -> anyhow::Result<Vec<Segment>> {
        ...
        self.run_raw(&pcm_data)
    }

    pub fn run_raw(&mut self, pcm_data: &[f32]) -> anyhow::Result<Vec<Segment>> {
        let device = Device::Cpu;
...
impl yew_agent::Worker for Worker {
    ...
    fn handle_input(&mut self, msg: Self::Input, id: HandlerId) {
        let output = match msg {
            ...
            WorkerInput::DecodeTaskRaw { samples } => match &mut self.decoder {
                None => Err("model has not been set".to_string()),
                Some(decoder) => {
                    decoder.run_raw(&samples).map(WorkerOutput::Decoded).map_err(|e| e.to_string())
                }
            },
        };
