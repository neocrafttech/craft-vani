...
fn log_mel_spectrogram_w<T: Float>(
    ith: usize, hann: &[T], samples: &[T], filters: &[T], fft_size: usize, fft_step: usize,
    speed_up: bool, n_len: usize, n_mel: usize, n_threads: usize,
) -> Vec<T> {
    ...
    for i in (ith..n_len).step_by(n_threads) {
        ...
        for j in 0..fft_size {
            fft_in[j] =
                if offset + j < samples.len() { hann[j] * samples[offset + j] } else { zero }
        }
...
fn log_mel_spectrogram_<T: Float + std::fmt::Display>(
    samples: &[T], filters: &[T], fft_size: usize, fft_step: usize, n_mel: usize, speed_up: bool,
) -> Vec<T> {
    let zero = T::zero();
    let two_pi = T::PI() + T::PI();
    let half = T::from(0.5).unwrap();
    ...
    let n_len = if !n_len.is_multiple_of(pad) { (n_len / pad + 1) * pad } else { n_len };
    let n_len = n_len + pad;
    let samples = {
...
pub fn pcm_to_mel<T: Float + std::fmt::Display>(
    cfg: &worker::m::Config, samples: &[T], filters: &[T],
) -> anyhow::Result<Vec<T>> {
